...
def memphis_tg():
    ...
    build_parser.add_argument("-t", "--with-ht", help="Build RTD scenarios w/ HT", action="store_true", default=False)
    build_parser.add_argument("-m", "--with-mapp", help="Build RTD scenarios w/ Mapp", action="store_true", default=False)
    ...
    sim_parser.add_argument("-t", "--with-ht", help="Simulate RTD scenarios w/ HT", action="store_true", default=False)
    sim_parser.add_argument("-m", "--with-mapp", help="Simulate RTD scenarios w/ Mapp", action="store_true", default=False)
    ...
    ext_ht_parser.add_argument("-t", "--with-ht", help="Extract RT detection dataset with HT", action="store_true", default=False)
    ...
    if args.option == "generate":
        ...
        generator = Generator(MEMPHIS_V_PATH, args.APPLICATION, args.proportion, args.rtd, mc_size=args.size, fp_rtd=args.fp_rtd, mal_app=args.mal_app)
        generator.write(args.output)
    elif args.option == "build":
        builder = Builder(args.TESTCASE, args.APPLICATIONS, args.SCENARIOS, args.no_base, args.with_ht, args.with_mapp)
        builder.build()
    elif args.option == "simulate":
        simulator = Simulator(args.TESTCASE, args.no_base, args.with_ht, args.with_mapp)
        simulator.simulate()
    elif args.option == "extract":
        extractor = Extractor(args.TESTCASE, args.no_base, args.with_ht, False, appid=args.appid)
        extractor.extract()
    else:
        parser.print_usage()
